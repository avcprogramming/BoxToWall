...
namespace AVC
{
  ...
  Wall
  {
    ...
    /// <summary>
    /// Текущий выбранный язык плагинов на момент обращения к веб-серверу. 
    /// En, Ru, It, Ge, Zh
    /// </summary>
    [DataMember]
    public string
    ...
    /// <summary>
    /// Толщина материала-каркаса стены. Берется у материала исходного солида-бокса. 
    ...
    /// Берется у материала передней грани исходного солида-бокса. 
    ...
    /// Берется у материала задней грани исходного солида-бокса. 
    /// Можно назначить только через Палитру Свойств AVC в свойствах покрытия.
    /// Если 0 - надо оставить каркас без облицовки
    /// Если не задан материал, то по умолчанию -1 (веб-сервер пусть сам решает какую толщину делать)
    /// </summary>
